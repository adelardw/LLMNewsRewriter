...
def is_junk_post_regex(text: str) -> bool:
    """
    Проверяет пост на рекламу и мусор с помощью Regex.
    Возвращает True, если пост нужно отбросить.
    """
    if not text:
        return True

    # Список паттернов. Можно легко дополнять.
    patterns = [

        r"erid\s*:",               
        r"идентификатор\s+рекламы",
        

        r"на\s+правах\s+рекламы",
        r"#\s*реклама",             
        r"партн[её]рский\s+материал",
        r"сделать\s+ставку",
        r"букмекер",
        r"казино",
        r"1win",                    
        ...
        r"арбитраж\s+трафика",
        r"заработок\s+в\s+интернете",
        r"обучаю\s+заработку",
        r"пишите\s+в\s+(?:лс|личку|директ)", 
        ...
        r"читать\s+(?:далее|полностью|в\s+источнике)",
        r"продолжение\s+(?:тут|здесь|в\s+канале)",
        r"ссылка\s+в\s+(?:описании|внизу|комментариях)",
        r"кнопка\s+ниже",
        r"подпи(?:шись|сывайся|счик)",
        r"доступ\s+закрыт",
        r"подать\s+заявку",
        r"только\s+для\s+подписчиков"
    ]
    ...
    # Ищем совпадение. 
    # flags=re.IGNORECASE делает поиск нечувствительным к регистру (А = а)
    # flags=re.MULTILINE позволяет искать по строкам, если нужно (тут не критично, но полезно)
