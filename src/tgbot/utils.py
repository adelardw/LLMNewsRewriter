...
def is_junk_post_regex(text: str) -> bool:
    ...
    patterns = [
        ...
        # --- 2. РЕКЛАМА И СПАМ ---
        # Ловит: рекламА, рекламЫ, рекламЕ, рекламНЫЙ...
        r"реклам[а-яё]*",   
        
        # Ловит: партнерский, партнерского, партнерская...
        r"партн[её]рск[а-яё]*", 
        ...
        # Ловит: заработок, заработать, заработай
        r"заработ[а-яё]*\s+в\s+интернет[а-яё]*", 
        r"арбитраж[а-яё]*",
        r"темк[а-яё]*", # Сленг "темки"

        # Ловит: пишите в лс, писать в личку, пиши в директ
        r"пиш[а-яё]*\s+в\s+(?:лс|личк[а-яё]*|директ)", 

        # --- 3. КОНКУРСЫ И РОЗЫГРЫШИ (Самое важное) ---
        # Корень "розыгрыш" + любые буквы (е, а, ей, и...)
        r"розыгрыш[а-яё]*", 
        
        # Корень "конкурс" + (ы, ов, е...)
        r"конкурс[а-яё]*",  
        
        # Корень "гив" (с проверкой границы слова \b, чтобы не задеть "погив")
        r"\bгив[а-яё]*",    
        r"giveaway",
        
        # Корень "подар" -> подарок, подарки, подарим, подарочный
        r"подар[а-яё]*",    
        
        # Корень "выигр" -> выиграй, выиграть, выигрыш
        r"выигр[а-яё]*",    
        
        # Корень "бесплатн" -> бесплатно, бесплатный
        r"бесплатн[а-яё]*", 
        
        # Осторожно с короткими корнями!
        # "Приз" -> призы, призов. (Используем \b, чтобы не задеть "признак")
        r"\bприз(?:[а-яё]+)?\b", 

        # --- 4. БАЙТ НА ПОДПИСКУ / ДЕЙСТВИЯ ---
        # Корень "подпи" ловит: подпиСКА, подпиШИСЬ, подпиСЧИК, подпиСЫВАЙСЯ
        r"подпи(?:с|ш)[а-яё]*", 
    ...
    # Флаги:
    # re.IGNORECASE (I) - чтобы ловить "РОЗЫГРЫШ", "Розыгрыш", "розыгрыш"
    # re.MULTILINE (M) - для корректной работы с многострочным текстом
