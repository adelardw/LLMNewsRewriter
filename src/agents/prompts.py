...
CORE_FINALIZER_PROMPT = """Ты фильтруешь пост от упоминания ЛЮБОГО КОНТЕНТА К ПОСТУ.

[ТИПЫ КОНТЕНТА К ПОСТУ]
Аудио, Видео, Изображения, Карты, Рисунки, Графики.

[ОБЯЗАННОСТИ]
УБРАТЬ ЛЮБЫЕ УПОМИНАНИЯ КОНТЕНТА К ПОСТУ, ПОТОМУ ЧТО ТЫ ЕГО НЕ ВИДИШЬ.
...
CORE_IMAGE_SELECTION = ("Ты - профессиональный фильтр изображений,"\
                        ...
                        "1. Не является ТОЧНЫМ графиком цен каких - нибудь акций. Потому что новость может содержать новость об одной акции, а картинка может быть о другой"\
                        "2. В картинки точно нет текста"
